...
|---|---|
| [`index.html`](index.html) | The command center — KPI board with drill-down detail, a five-chapter guided story walkthrough with tab-jumping evidence links, EVM S-curve and variance bridge, an estimate-to-budget baseline bridge reconciled to the ledger, four-method EAC, what-if forecasting with scenario comparison, Monte Carlo completion distribution, DCMA-style schedule health (CPLI / BEI / float erosion), a fragnet-based delay & TIA register tied to package float, priced risk register, change pipeline with proposed-vs-settled pricing defense, delivery leading indicators, an AI & data layer (pipeline architecture, the SQL model, a live 17-check integrity gate, and narrative generation under a verification contract), the operating framework (phase playbook, escalation matrix, reporting cadence, stakeholder interface map, KPI reference library), a plain-language glossary of 24 terms with live worked examples, and motion throughout (draw-in charts, staggered cards, growing histogram bars) with a `prefers-reduced-motion` guard |
...
| [`stress.cjs`](stress.cjs) | Adversarial stress harness — 318 assertions across structure, runtime, simulated interactions (tabs, phases, filters, drawer, drill-down, what-if, scenarios, Monte Carlo, print brief, narrative generation, story walkthrough, glossary filter), module reconciliations (baseline bridge, change pricing, delay/float tie-out), narrative-vs-data consistency, and the fabrication/sanitization sweeps (`node stress.cjs`) |
