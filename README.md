...
|---|---|
| [`index.html`](index.html) | The command center — KPI board with drill-down detail, a five-chapter guided story walkthrough with tab-jumping evidence links, EVM S-curve and variance bridge, four-method EAC, what-if forecasting with scenario comparison, Monte Carlo completion distribution, DCMA-style schedule health (CPLI / BEI / float erosion), priced risk register, change pipeline, delivery leading indicators, an AI & data layer (pipeline architecture, the SQL model, a live 14-check integrity gate, and narrative generation under a verification contract), the operating framework (phase playbook, escalation matrix, reporting cadence, KPI reference library), a plain-language glossary of 21 terms with live worked examples, and motion throughout (draw-in charts, staggered cards, growing histogram bars) with a `prefers-reduced-motion` guard |
