...
| Additional fit-brief and case pages | Requirement-coverage briefs and a data-center investment case written for specific audiences. They use public postings and company facts (and say so), are `noindex`, and are not linked from the dashboard. Each has its own `verify-*.cjs` tie-out harness. |
...
| [`verify-facade.cjs`](verify-facade.cjs) | Tie-out harness for it — 309 assertions. Re-derives every package total from the raw ledger literals rather than reading computed values back, and pins the two identities the method could be silently wrong about: **quantity + price = total** (exactly, per elevation and in aggregate) and **shop EV + field EV = package EV**. Also asserts the page's own narrative claims against the data — an early version caught the cross-domain "one elevation is worst on all three" claim being tested against the wrong metric (`node verify-facade.cjs`) |
| [`verify-exposure.cjs`](verify-exposure.cjs) | Exposure guard — 24 checks that keep employer-identifying and interview-coaching text out of the dashboard, README and published site, and confirm harnesses and docs are excluded from Pages (`node verify-exposure.cjs`) |
...
**The program, packages, milestones and risks are invented.** The dashboard and its data layer use no client, employer, or agency data. The separate fit-brief pages cite public postings and company facts and say so on the page. The methods are the content.
