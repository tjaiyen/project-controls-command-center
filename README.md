...
**20 KPIs · 13 tabs · 4,094 tests passing · 122 independent SQL parity checks · zero dependencies**
...
*The dashboard's own 11-stop guided Tour, narrated stop by stop — not a scripted demo recording,
...
- **Overview** — a "Six lenses, not one blended score" card (why Cost/Schedule/Risk/Change/
  ...
  "change one input, watch the KPIs move" demo, an 11-stop guided Tour with tab-jumping evidence
...
- **Cost** — EVM S-curve and variance bridge, an estimate-to-budget baseline bridge reconciled to
  ...
  direct-fixation track, real discount-rate methodology, explicitly illustrative dollar inputs); a
  WSDOT-style cost-code / unit-price bid-item breakdown one level below the FTA SCC categories
  (real TxDOT-sourced concrete/excavation unit rates, 3 honest gap lines where no verified public
  rate exists rather than an invented number, a real quantity-vs-price variance decomposition, a
  real 25%-quantity-overrun contract-adjustment flag, and 2 real Sound Transit/LA Metro reference
  benchmarks explicitly labeled as bundled/all-in figures) with a Pareto cost-driver ranking one
  level below the package-level variance bridge.
- **Schedule** — DCMA-style schedule health, named explicitly (SPI(t)/Earned Schedule, CPLI, BEI,
  ...
  revenue-service drift with that delay's own fragnet removed — revenue-service forecast drift, and
  a labor-availability leading indicator (3 real driving-schedule trades checked for mobilization
  lead-time confirmed vs. required, sourced from AGC/ABC workforce research).
- **Risk & Change** — a priced risk register with a click-through per-risk drill-down drawer (7
  risks, including an extreme-weather exposure item priced regardless of federal policy churn), a
  forward material-price exposure trigger extending the Contingency Drawdown Index's own logic
...
- **AI & Data** — pipeline architecture (now click-through, every node opening a story card naming
  the live field it feeds), a stakeholder data-readiness register (6 real external agencies scored
  against Wang's 6-force interagency data-sharing model, each paired with a named next move drawn
  from Carnegie's *How to Win Friends and Influence People* — or, where the real blocker is legal
  authority rather than willingness, explicitly flagged as outside persuasion's reach), the SQL
  model, a live 30-check integrity gate (the 29th requires a
  logged root cause before any Quality NCR can be marked closed; the 30th flags any package whose
  GC-claimed progress exceeds ledger-verified progress by more than 5%), an embodied-carbon
...
- **Operating Framework** — a phase playbook, a WBS/CBS/OBS/ABS control-account mapping, Board
  phase-gate governance with a live Gate-5 hard stop, a pending owner/agency decisions register
  (its own aging clock, feeding the triage queue as a 5th independent source), an escalation
  matrix where every rule now states why its threshold sits where it does, a 3-lever Gate 5
  solvency what-if sandbox
...
- **Glossary** — 76 terms with live worked examples, a click-driven inline "i" help icon next to
...
| [`otak.html`](otak.html) | Fit brief: requirement-by-requirement coverage against a Project Controls Manager posting, gaps included. Re-verified against live req #3775557 on 26 Aug 2026 |
| [`walters-wolf.html`](walters-wolf.html) | Fit brief, second audience: capability coverage for project controls at a **unitized curtain wall subcontractor**, gaps included. No posting exists behind it, so the capability list is self-authored and the page says so in its own lede rather than implying an external rubric. Company facts verified 2 Sep 2026 |
| [`verify-walters-wolf.cjs`](verify-walters-wolf.cjs) | Tie-out harness for that page — stubs the DOM, executes its IIFE, and asserts the rendered coverage table and summary strip against the `CAPS` array they derive from (`node verify-walters-wolf.cjs`) |
| [`dc-investment-case.html`](dc-investment-case.html) | An interactive rebuild of a full data-center investment case (originally a 14-slide deck) linked from the fit brief above — live SVG charts instead of static images, a filterable 6-account GC table, an expandable 11-row risk register, a 4-phase roadmap timeline, and a 3-way "if wrong / if right / if wait" decision toggle. Rebuilt from the source proposal's underlying chapters, not the deck's slide text, after an audit found two slides with fabricated content (invented building sales and bank stories, three GC names with zero basis in the ~230-source research) |
| [`docs/Data_Center_Investment_Case_Walters_Wolf.pptx`](docs/Data_Center_Investment_Case_Walters_Wolf.pptx) | The corrected slide deck itself, kept as a secondary download alongside the interactive page above |
| [`verify-dc-investment-case.cjs`](verify-dc-investment-case.cjs) | Static-analysis harness for the page above — not a DOM-execution harness like the others (the page builds its charts via `appendChild`/`createElementNS`, which a hand-stubbed DOM can't faithfully run, and this repo stays zero-dependency rather than pull in `jsdom` for one page). Pins the fabricated-content sanitization sweep, every data array's entry count, and nav/section parity (`node verify-dc-investment-case.cjs`); full interactive-behavior testing (chart rendering, filters, keyboard interaction, theme-toggle re-render) was done via live browser execution, not simulated here |
| [`facade.html`](facade.html) | **Unitized curtain wall controls** — the same engine pointed at a facade subcontractor's actual problem rather than a capital program's. Panel-level earned value on two rules-of-credit ladders (shop and field, weighted by the real cost split), a factory-vs-field dual S-curve whose shaded gap *is* the buffer, days-of-cover as a leading indicator with its own trend, quantity/price/productivity variance decomposition against the bid, AAMA/ASTM quality gates evaluated live (one is genuinely blocked), and tolerance creep projected against the joint budget. Synthetic package, `noindex` not applied — this one is meant to be read |
| [`verify-facade.cjs`](verify-facade.cjs) | Tie-out harness for it — 79 assertions. Re-derives every package total from the raw ledger literals rather than reading computed values back, and pins the two identities the method could be silently wrong about: **quantity + price = total** (exactly, per elevation and in aggregate) and **shop EV + field EV = package EV**. Also asserts the page's own narrative claims against the data — an early version caught the cross-domain "one elevation is worst on all three" claim being tested against the wrong metric (`node verify-facade.cjs`) |
| [`pipeline/`](pipeline/) | The data layer made executable — `run_pipeline.py` synthesizes raw monthly claims deterministically, builds the ledger through `models/fct_control_account.sql` in DuckDB, enforces every guardrail declared in `models/schema.yml`, and proves the SQL output identical to the browser's JavaScript derivation (122 checks, emitted as a structured `checks{total,passed,failed}` field on `pipeline/output/ledger.json`). The raw claim rows are synthesized to sum back to the dashboard's own real PV/EV/AC totals, so the proof covers the SQL aggregation/formula layer, not an independently-entered dataset. Requires `pip install duckdb` — no other dependencies. Set up `pipeline/.venv` once (`python3 -m venv pipeline/.venv && pipeline/.venv/bin/pip install duckdb`) and `stress.cjs` will run this live and verify index.html's own "122 parity checks" claim against it, instead of a static text match |
