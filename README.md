...
**20 KPIs · 13 tabs · 3,997 tests passing · 65 independent SQL parity checks · zero dependencies**
...
- **Glossary** — 73 terms with live worked examples, a click-driven inline "i" help icon next to
...
| [`verify-facade.cjs`](verify-facade.cjs) | Tie-out harness for it — 58 assertions. Re-derives every package total from the raw ledger literals rather than reading computed values back, and pins the two identities the method could be silently wrong about: **quantity + price = total** (exactly, per elevation and in aggregate) and **shop EV + field EV = package EV**. Also asserts the page's own narrative claims against the data — an early version caught the cross-domain "one elevation is worst on all three" claim being tested against the wrong metric (`node verify-facade.cjs`) |
| [`pipeline/`](pipeline/) | The data layer made executable — `run_pipeline.py` synthesizes raw monthly claims deterministically, builds the ledger through `models/fct_control_account.sql` in DuckDB, enforces every guardrail declared in `models/schema.yml`, and proves the SQL output identical to the browser's JavaScript derivation (65 checks, emitted as a structured `checks{total,passed,failed}` field on `pipeline/output/ledger.json`). The raw claim rows are synthesized to sum back to the dashboard's own real PV/EV/AC totals, so the proof covers the SQL aggregation/formula layer, not an independently-entered dataset. Requires `pip install duckdb` — no other dependencies. Set up `pipeline/.venv` once (`python3 -m venv pipeline/.venv && pipeline/.venv/bin/pip install duckdb`) and `stress.cjs` will run this live and verify index.html's own "65 parity checks" claim against it, instead of a static text match |
