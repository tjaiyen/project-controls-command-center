...
| [`pipeline/`](pipeline/) | The data layer made executable — `run_pipeline.py` synthesizes raw monthly claims deterministically, builds the ledger through `models/fct_control_account.sql` in DuckDB, enforces every guardrail declared in `models/schema.yml`, and proves the SQL output identical to the browser's JavaScript derivation (66 checks, emitted as a structured `checks{total,passed,failed}` field on `pipeline/output/ledger.json`). The raw claim rows are synthesized to sum back to the dashboard's own real PV/EV/AC totals, so the proof covers the SQL aggregation/formula layer, not an independently-entered dataset. Requires `pip install duckdb` — no other dependencies. Set up `pipeline/.venv` once (`python3 -m venv pipeline/.venv && pipeline/.venv/bin/pip install duckdb`) and `stress.cjs` will run this live and verify index.html's own "66 parity checks" claim against it, instead of a static text match |
